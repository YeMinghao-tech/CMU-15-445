...
TEST(TxnIndexTest, DISABLED_IndexConcurrentInsertTest) { // NOLINT
  ...
  for (int n = 0; n < trials; n++) {
    ...
    for (int thread = 0; thread < thread_cnt; thread++) {
      insert_threads.emplace_back([&bustub, thread, generate_sql, &result_mutex,
                                   &operation_result]() {
        NoopWriter writer;
...
TEST(TxnIndexTest, DISABLED_IndexConcurrentUpdateTest) { // NOLINT
  ...
  const auto generate_txn_insert_sql = [](int orig_value,
                                          int n) -> std::string {
  ...
  for (int n = 0; n < trials; n++) {
    ...
    global_disable_execution_exception_print.store(true);
    for (int thread = 0; thread < thread_cnt; thread++) {
      update_threads.emplace_back([add_delete_insert, &bustub, thread,
                                   generate_sql, generate_select_sql,
                                   generate_delete_sql, generate_txn_insert_sql,
        ...
        for (int i = 0; i < number_cnt; i++) {
          ...
          if (add_delete_insert) {
            ...
            BUSTUB_ENSURE(
                bustub->ExecuteSqlTxn(generate_select_sql(i), data_writer, txn),
                "cannot retrieve data");
            BUSTUB_ENSURE(data_writer.values_.size() == 1,
                          "more than 1 row fetched??");
            const auto b_val = std::stoi(data_writer.values_[0][0]);
            BUSTUB_ENSURE(
                bustub->ExecuteSqlTxn(generate_delete_sql(i), data_writer, txn),
                "cannot delete data");
            BUSTUB_ENSURE(
...
TEST(TxnIndexTest, DISABLED_IndexConcurrentUpdateAbortTest) { // NOLINT
  ...
  for (int n = 0; n < trials; n++) {
    ...
    global_disable_execution_exception_print.store(true);
    for (int thread = 0; thread < thread_cnt; thread++) {
      update_threads.emplace_back([&bustub, thread, generate_sql, &result_mutex,
                                   &operation_result]() {
        NoopWriter writer;
